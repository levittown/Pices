...
/* 
   utils.h
   $Id: utils.h,v 1.12 2003/01/31 22:51:05 eschrich Exp $
...
#define __UTILS_VERSION  "$Id: utils.h,v 1.12 2003/01/31 22:51:05 eschrich Exp $"
   ...
   #error "CALLOC previously defined, not re-defined"
    ...
    #define REALLOC(q, n) \
         ({ \
	      ...
	      fprintf(stderr, "%s:%d error re-allocating memory\n", \
                                                       __FILE__, __LINE__); \
              exit(1); \
            } \
            __p;  \
    ...
    #define FOPEN(filename, mode) \
       ({ \
          ...
          if ( (__fp=fopen(__fstr,__mstr)) == 0 ) { \
            fprintf(stderr,"%s:%d\n Error opening %s (mode %s): %s\n", \
	                                     __FILE__, __LINE__, \
		                             __fstr, __mstr,strerror(errno)); \
            exit(1); \
          } \
          __fp; \
       })
...
#else
 #error "Macros are implemented as functions, please include utils.o"
 FILE *FOPEN();
 void *MALLOC();
 void *CALLOC();
 void *REALLOC();
