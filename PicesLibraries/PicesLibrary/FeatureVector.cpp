...
kkint32 FeatureVector::MemoryConsumedEstimated ()  const
{
  ...
  if  (featureData)
    memoryConsumedEstimated += sizeof (FVFloat) * numOfFeatures;
...
void  FeatureVector::ResetNumOfFeatures (kkint32  newNumOfFeatures)
{
  ...
  kkint32  x;
  FVFloat*  newFeatureData = new FVFloat[newNumOfFeatures];
...
void  FeatureVector::AllocateFeatureDataArray ()
{
  ...
  featureData = new FVFloat [numOfFeatures];
...
FVFloat  FeatureVector::FeatureData (kkint32 featureNum)  const
{
...
bool  FeatureVector::FeatureDataValid ()
{
  ...
  {
    if  ((featureData[featureNum] == FloatMin)  ||  (featureData[featureNum] == FloatMax))
      return  false;
  }
...
kkint32  FeatureVectorList::GetClassCount (MLClassConstPtr  c)  const
{
  kkint32  count =0;
  FeatureVectorList::const_iterator  idx;
  for  (idx = begin ();  idx != end ();  idx++)
...

{
  ...
  for  (idx = begin ();  idx !=  end ();  idx++)
  {
    ...
    for  (x = 0;  x < symbolicFields.size ();  x++)
    {
      ...
      i->AddFeatureData (fieldNum, FVFloat (newCode));
    }
  }
