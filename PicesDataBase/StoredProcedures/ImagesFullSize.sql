...
drop procedure  if exists ImagesFullSizeDelete;
delimiter //

create procedure ImagesFullSizeDelete (in  _ImageFileName   varChar(64))
...
begin
  ...
  if  (!isnull(_imageId)  and  (_imageId > 0))  then
    delete  from  ImagesFullSize  where  ImageId = _imageId;
  end if;
