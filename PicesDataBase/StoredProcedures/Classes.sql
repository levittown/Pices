...
begin
  ...
  if  @srcClassId  is null  then
    select  "MLClassDeleteComprehensive", _className;
  end if;
  ...
  set  @destClassName = (select c1.ClassName  from  Classes c1  where  c1.ClassId   = @destClassId);
  
  select "Deleting_Class", concat(_className, "(", @srcClassId, ")"), concat(@destClassName, "(", @destClassId, ")");
  
  call  MLClassMergeComprehensive(_className, @destClassName);
