  ...
  for  l in sipperFilesData:
    ...
    if  len(fields) > 1:
      ...
      for  (sipperFileId)  in  c:
        sipperFileDic[sipperFileIdExt] = sipperFileId
...
def  ImportLogEntries(dirName):
  ...
  for  l in logEntryData:
    ...
    if  len(fields) > 13:
      ...
      if  (LogEntryExist (db, c, logEntryIdExt, progName, cmdLine, DateTimeFromStr(dateTimeStart))):
        print(str(logEntryIdExt) + " already in database.")
      else:
        sqlStr = ("insert into LogEntries(ProgCode, ProgName, DateTimeCompiled, CmdLine, CompName, DataBaseUserName, " +
                  "CompUserName, DateTimeStart, DateTimeStartUtc, CpuTimeUsed, CompletionStatus, SipperFileId, externalLogEntryId)" + " " +
           "values(" + progCode             + "," +
                       progName             + "," +
                       dateTimeCompiled     + "," +
                       cmdLine              + "," +
