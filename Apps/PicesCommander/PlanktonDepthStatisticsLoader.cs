...
{
  public class PlanktonDepthStatisticsLoader
  {
    public  delegate  void  AlertCompletion (PlanktonDepthStatisticsLoader  sender);
    ...
    public  PlanktonDepthStatisticsLoader (PicesDataBaseImageGroup  _selectedImageGroup,
                                           String                   _cruiseName,
                                           String                   _stationName,
                                           String                   _deploymentNum,
                                           String                   _sipperFileName,
                                           float                    _depthIncrements,
                                           PicesClass               _mlClass,
                                           char                     _classKeyToUse,
                                           float                    _probMin,
                                           float                    _probMax,
                                          ...
                                          )
    {
      ...
      depthIncrements      = _depthIncrements;
      mlClass           = _mlClass;
    ...
    {
      uint[] statsToReturn = null;
      blocker.StartBlock (); 
      if  (loadedStats != null)
      {
        statsToReturn = new uint[loadedStats.Length];
        for  (int x = 0;  x < loadedStats.Length;  x++)
          statsToReturn[x] = loadedStats[x];
      }
      blocker.EndBlock ();
    ...
    private  void  AddNextBunchIntoLoadedStats (ref uint[] loadedStats,
                                                uint[]     nextBunch,
                                                ref uint   imageCountTotal
                                               )
    {
      if  (loadedStats == null)
      {
        loadedStats = new uint[nextBunch.Length];
      }
      else
      {
        if  (nextBunch.Length > loadedStats.Length)
        {
          uint[]  newLoadedStats = new uint[nextBunch.Length];
          for  (int x = 0; x < loadedStats.Length;  x++)
            newLoadedStats[x] = loadedStats[x];
          loadedStats = newLoadedStats;
        }
      }
      {
        for  (int x = 0;  x < nextBunch.Length;  x++)
        {
          loadedStats[x] += nextBunch[x];
          imageCountTotal += nextBunch[x];
        }
      }
