...
void  ShuffleData (FeatureVectorListPtr  examples)
{
  ...
  FeatureVectorListPtr  shuffledData = 
          examples->StratifyAmoungstClasses (&imageClasses, lopOff, numOfFolds);
...
void  SplitExamples (FeatureVectorList*  examples)
{
  ...
  FeatureVectorListPtr  file1Shuffeled
         = file1Examples.StratifyAmoungstClasses (&imageClasses, lopOff, numOfFolds);

  FeatureVectorListPtr  file2Shuffeled
         = file2Examples.StratifyAmoungstClasses (&imageClasses, lopOff, numOfFolds);
...
void  CreateTrainAndTestDataSets (FeatureVectorListPtr  examples)
{
  ...
  for  (int cIDX = 0;  cIDX < numOfClasses;  cIDX++)
  {
    ...
    classExamples = examples->ExtractImagesForAGivenClass (imageClasses.IdxToPtr (cIDX), lopOff, -1);
    ...
    FeatureVectorListIterator iIDX (*classExamples);
    FeatureVectorPtr  image = NULL;

    for  (iIDX.Reset ();  (image = iIDX.CurPtr ());  ++iIDX)
    {
      if  (trainingExamples.size () < (uint)maxNumOfTrainExamples)
...

  {
    if  (numOfFolds > 1)
    {
      FeatureVectorListPtr  testExamplesStratified = trainingExamples.StratifyAmoungstClasses (&imageClasses, -1, numOfFolds);
      uint  numExamplesWritten = 0;
...
void  CreateFolds (int  numOfFolds)
{
  ...
  for  (idx = stats->begin ();  idx != stats->end ();  idx++)
  {
	...
	ImageClassPtr c = (*idx)->ImageClass ();
...
int  main (int argc, char **argv)
{
  ...
  if  (deleteDuplicates)
  {
    ...
    if  (dupDetector.DupImages ()->QueueSize () > 0)
    {
      ...
      dupDetector.PurgeDuplicates (NULL);
